...
export default function Hero() {
  ...
  // Image stays still while bridge text rises (0.34→0.62 = text entering from below over still image).
  // Only when bridge text reaches screen centre (0.62) does image begin to move — and at half
  // the rate, so bridge text visibly overtakes it. Mirrors Tresmares parallax exactly.
  const imageY             = useTransform(scrollYProgress, [0.62, 0.90],          [0, -80]);
  const bottomBlurOpacity  = useTransform(scrollYProgress, [0.50, 0.72],          [0, 1]);
